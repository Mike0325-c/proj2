...
&WallFollower::scan_callback, \
   ...
   ************************************************************/
   ...
   RCLCPP_INFO(this->get_logger(), "Wall follower node has been initialised starting now!");
}
...
{   
    ...
    if (scan_data_[FRONT] > 0.55) {
       if (scan_data_[LEFT_FRONT] < 0.43 && scan_data_[LEFT_BACK] < 0.43) {
          if (scan_data_[LEFT_FRONT] < 0.47 && scan_data_[FRONT_LEFT] < 0.47) {
            update_cmd_vel(0.1, -0.4);
          }
           ...
           update_cmd_vel(0.1, -0.35); // turn right when too close left
           return;
       } 
       return;
    } else if (scan_data_[FRONT] < 0.55) { //when there is obsticles
       if (scan_data_[FRONT] < 0.5) { //too close to the front wall
           update_cmd_vel(0,-0.5);
           return;
       } else if (scan_data_[FRONT_LEFT] < 0.5 && scan_data_[FRONT_RIGHT] < 0.5) { //when there is wall in front
           update_cmd_vel(0.1,-1);
           return;
       } else {
